...
var handleAuthentication = function(req, options) {
  ...
  options || (options = {});
  if(req.headers.authorization && req.headers.authorization.lastIndexOf('Basic ',0) === 0) {
    var md5hash = req.headers.authorization.substring('Basic '.length);
    var credentials = atob(md5hash).split(':');
