  ...
  ldap.authenticate(username, password, function(err, user) {
    ...
    if (err) {
      ...
      if (err.name === 'InvalidCredentialsError' || err.name === 'NoSuchObjectError' || (typeof err === 'string' && err.match(/no such user/i))) {
         ...
         * 533, 534	account disabled The user has not been granted the requested logon type at this machine
         * 701	account expired
         * 773	user must reset password
         * 775	user account locked
         */
        var ldapComment = err.message.match(/data ([0-9a-fA-F]*), v[0-9a-fA-F]*/);
        var ldapContextError = ldapComment[1] || 0;
        var message;
        switch(ldapContextError){
          case '530': message = options.invalidLogonHours || 'Not Permitted to login at this time';break;
          case '531': message = options.invalidWorkstation || 'Not permited to logon at this workstation';break;
          case '532': message = options.passwordExpired || 'Password expired';break;
          case '533':
          case '534': message = options.accountDisabled || 'Account disabled';break;
          case '701': message = options.accountExpired || 'Account expired';break;
          case '773': message = options.passwordMustChange || 'User must reset password';break;
          case '775': message = options.accountLockedOut || 'User account locked';break;
          case '525':
          case '52e':
          default: message = options.invalidCredentials || 'Invalid username/password';break;
        }
